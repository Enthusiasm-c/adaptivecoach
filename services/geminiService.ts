...
function buildInitialPrompt(profile: OnboardingProfile): string {
    ...
    2. Выбор упражнений:
        - Приоритет базовым движениям + ОБЯЗАТЕЛЬНО изолированные на мелкие группы.
        - ОБЯЗАТЕЛЬНО включи: 1-2 упражнения на бицепс (сгибания рук), 1-2 на трицепс (разгибания/французский жим).
...
2. Стрик (#${currentStreak}) и номер тренировки (#${workoutNumber}) — РАЗНЫЕ числа! Стрик = тренировок БЕЗ пропусков подряд. Не объединяй их в одно предложение если они отличаются.
