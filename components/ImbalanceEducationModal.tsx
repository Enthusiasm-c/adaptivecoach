...
function getEducationalContent(type: string): EducationalContent {
  switch (type) {
    ...
    case 'anterior_posterior':
      return {
        title: 'Передняя/Задняя цепь',
        whatIsIt:
          'Дисбаланс между передней цепью (квадрицепсы) и задней (ягодицы, бицепс бедра). Часто возникает при избытке приседаний и недостатке тяг.',
        risks: [
          'Боли в пояснице',
        ...
        howToFix:
          'Добавь упражнения на заднюю цепь. Соотношение становая:присед должно быть примерно 1.2:1.',
